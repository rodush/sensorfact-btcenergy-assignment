...
export class CacheService {
  ...
  public async get<T>(key: string): Promise<T | null> {
    ...
    console.debug(`Cache ${serialized ? 'HIT' : 'MISS'} for key: ${key}. Cache size: ${this.cache.size}`);
  ...
  public async set<T>(key: string, value: T, ttlSeconds?: number): Promise<void> {
    try {
        ...
        console.debug(`Cache SET for key: ${key}. New cache size: ${this.cache.size}. TTL: ${ttlSeconds ? ttlSeconds + 's' : 'default'}`);  
    } catch (error) {
